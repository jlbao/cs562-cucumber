Feature: Cash Withdrawal
  ...
  Scenario Outline: Withdraw cash successfully twice
    Given that I have a starting balance of $<start_balance> in my account
    ...
    Examples:
    | start_balance	| amount1	| amount2	| end_balance	|
    | 100.00		| 20		| 40		| 40.00			|
    | 50.00			| 30		| 20		| 0.00			|
    | 51.50			| 30		| 20		| 1.50			|
...
# 1. You cannot make more than three withdrawals in a single 24-hour period.
	...
	Then There will be an withdraw exceed 24-hour exception
...
#
# When there is a violation of the rules a message should be displayed to the user. You need
# to specify it in your scenario and then write the appropriate rules to ensure this. You probably
# will need to catch specific exceptions in your code and have some sort of mock display object that
